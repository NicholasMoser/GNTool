...
public class FPKFileHeader
{
    ...
    /**
     * Sets an offset for the header. Will only apply if the offset is not already set, which is represented by the integer -1.
     * 
    ...
    public byte[] getBytes()
    {
        ...
        return Bytes.concat(fileNameBytes, ByteUtils.intToBytes(0), ByteUtils.intToBytes(offset), ByteUtils.intToBytes(compressedSize), ByteUtils.intToBytes(uncompressedSize));
    }
}
