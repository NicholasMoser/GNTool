...
public class PRSCompressor
{
	...
	/**
	 * Partial Java re-implementation of the memcmp(...) function in C.
	 * Given two indices for the input bytes, checks that the 
	 * given size number of bytes are the same. 
	...
	/**
	 * Writes a bit to the current flag byte. If there are no more bits left
	 * in the current flag byte, creates a new flag byte at the current output
	 * index and writes to it.
	 * @param output The output bytes.
	...
	/**
	 * Compresses between 2 and 5 bytes reachable through a short search.
	 * This includes the bits 0 and 0 in the flag byte, which indicates that
	 * this byte is compressed and is short.
	 * @param output The output bytes.
	 * @param len
	 * @param posy
	 */
	...
	/**
	 * Compressed a long length of bytes.
	 * This includes the bits 0 and 1 in the flag byte, which indicates that
	 * this byte is compressed and is long.
	 * @param output The output bytes.
	 * @param len
	 * @param posy
	 */
	...
	/**
	 * Writes an uncompressed byte to the output.
	 * This includes the single bit 1 in the flag byte, which indicates that
