...
/**
 * An uncompressor for Eighting PRS compressed files. Takes in the full byte array of the file and returns the uncompressed byte stream. Thanks to Luigi Auriemma for porting to QuickBMS. Thanks to tpu
 * for originally writing it: http://forum.xentax.com/viewtopic.php?p=30387#p30387
 */
public class PRSUncompressor
{
    ...
    /**
     * Retrieves a number of bits from the flag byte. A single 1 means the next byte is uncompressed. A 0 followed by a 0 means that the next bytes are compressed via short search. A 0 followed by a 1
     * means that the next bytes are compressed via long search. Short searches will be followed by two bits used to calculate the length of the compressed bytes.
     * 
