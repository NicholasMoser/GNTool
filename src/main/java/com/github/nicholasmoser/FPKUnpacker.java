...
public class FPKUnpacker
{
    ...
    /**
     * Reads the header of the FPK file itself. This will return only the number of files from the header. Make sure to only call this method on a newly opened FPK file, since the header is first in
     * the file. This method will always read exactly 16 bytes.
     * 
    ...
    /**
     * Reads an individual file header from the FPK file. This will return the relevant FPKFileHeader object. Make sure to only call this method once you have already read the FPK header (first 16
     * bytes of the file). You will want to call this equivalent to the number of files contained in the FPK file. This method will always read exactly 32 bytes.
     * 
     * @param is The input stream to read it from.
     * @return The number of files in the FPK file.
     * @throws IOException If there is an exception relating to the FPK file input.
