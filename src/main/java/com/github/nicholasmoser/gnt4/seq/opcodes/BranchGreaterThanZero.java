...
public class BranchGreaterThanZero implements Opcode, BranchingOpcode {
  ...
  public byte[] getBytes(int offset, int size) {
    if (destination > offset && destination < offset + size) {
      return Bytes.concat(new byte[] {0x01, 0x35, 0x00, 0x00}, ByteUtils.fromInt32(destination + offset));
    }
    return getBytes();
  }
